---
title: eMaize_Tutorial
pubDatetime: 2018-04-12T23:30:17Z
description: "由于jupyter notebook的强大的展示功能，本教程还用jupyter notebook组织且运行，可以获得更好的学习效果，代码在这里,欢迎取用。在这里我简单介绍了如何配置jupyter，在Deep Learning tutorial中我也强烈推荐了jupyter，并且介绍了很多基于jupyter的资源，强烈建议尝试一下。"
author: "James Chen"
tags: ["machine learning", "techniques", "bioinformatics", "codes", "statistics"]
categories: ["machine learning"]
math: true
draft: false
---

这是为实验室写的，借由eMaize问题帮助大家简单了解机器学习基本方法和基础代码的教程。也可以在[这里](https://lulab.gitbooks.io/bioinfo/content/5%E5%A4%A7%E6%95%B0%E6%8D%AE%E6%95%B4%E5%90%88----%E6%9C%BA%E5%99%A8%E5%AD%A6%E4%B9%A0%E5%92%8C%E4%BA%BA%E5%B7%A5%E6%99%BA%E8%83%BD/51.html)看到

由于jupyter notebook的强大的展示功能，本教程还用jupyter notebook组织且运行，可以获得更好的学习效果，代码在[这里](https://github.com/james20141606/somethingmore/blob/master/bioinfo.ipynb),欢迎取用。[在这里](http://localhost:4000/2018/04/12/setup/)我简单介绍了如何配置jupyter，在[Deep Learning tutorial](https://james20141606.github.io/2018/04/10/Deep-Learning-Practice/)中我也强烈推荐了jupyter，并且介绍了很多基于jupyter的资源，强烈建议尝试一下。

## 0.背景简介
该通过基因型预测表型的实例来自[eMaize challenge](http://emaize.imaze.org):
eMaize问题要求我们以SNP作为特征，通过训练一个模型，对玉米的三个性状进行预测。
接下来的教程会展示从原始数据开始，如何对数据进行转换，存取，特征选择以及回归和后续分析的整个过程。本问题最基本的目标是使用6210个样本中的前4754个样本作为训练集，预测其他样本的性状<br>
...
![Markdown](http://i1.bvimg.com/640680/30af897795c31338.png)
...
可以跳过接下来的代码框步骤，直接使用处理好的结果，结果放在 /data/2bit_geno
```
...
- 过多的feature使得机器学习模型无法承受，一个考虑时间开销及效果的feature数量应该在几千至几万量级 <br>
...
**我们会提供ANOVA、Random projection处理上面5000个snps后的数据，以及在完整数据集上用Random projection降维至10000个feature的数据。用于送入下一部分的回归模型。下面的三种方法的处理后的数据可以在feature_selection文件夹下找到，存储格式为HDF5，可用h5py打开**
##### ANOVA数据：
...
```
![Markdown](http://i1.bvimg.com/640680/b8b10e4aac94c22f.png)
可以设定一个阈值，比如留下p-value前百分之四十的SNPs,根据index选取留下的SNPs
```
...
这里使用sklearn和xgboost提供的模块，这些模块具有很好的封装，使用风格统一，使用时可以查看其官方文档<br>
...
plt.tight_layout()
```
![Markdown](http://i1.bvimg.com/640680/53ed95bc3fc879c9.png)
![Markdown](http://i1.bvimg.com/640680/3a9fec1a694dc533.png)
![Markdown](http://i1.bvimg.com/640680/f9198785ddd4e809.png)
![Markdown](http://i1.bvimg.com/640680/18e028db8839992e.png)
...
```
![Markdown](http://i1.bvimg.com/640680/dc071479d62b84c7.png)
...
```
![Markdown](http://i1.bvimg.com/640680/b514a9c8b7cc7943.png)
![Markdown](http://i1.bvimg.com/640680/bbe789a8598188da.png)
![Markdown](http://i1.bvimg.com/640680/19a5441ffb4534eb.png)
...
![Markdown](http://i1.bvimg.com/640680/839f4f607631b772.png)
![Markdown](http://i1.bvimg.com/640680/4c1eb79af4bbb5fb.png)
可以看到一千次抽样的测试结果，大多数测试的PCC都比较高

![Markdown](http://i1.bvimg.com/640680/bea286d0f130d044.png)
按照样本查看每个样本多次抽样的平均PCC，注意这里是有bias没有消除的
![Markdown](http://i1.bvimg.com/640680/17df0a3dcaa71ada.png)

这里绘制了每个样本的平均PCC heatmap图像,可以发现大多数的样本是很好预测的，样本性状基本由父本性状主导(纵坐标为父本),但是少数亲缘关系较远的父本（图中蓝色线）就很难预测。
![Markdown](http://i1.bvimg.com/640680/50adb2cb87536a3d.png)
![Markdown](http://i1.bvimg.com/640680/48ee63cf0c78d296.png)
![Markdown](http://i1.bvimg.com/640680/d3bb77a76c162d53.png)
...
```
![Markdown](http://i1.bvimg.com/640680/818f1d13de885240.png)
